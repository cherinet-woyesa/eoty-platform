...
const ContentManager: React.FC = () => {
  ...
  const fetchUploads = async () => {
    try {
      setLoading(true);
      const response = await adminApi.getUploadQueue(statusFilter !== '' ? { status: statusFilter } : {});
  ...
  const handleApprove = async (uploadId: number, action: 'approve' | 'reject', rejectionReason?: string) => {
    try {
      ...
      const response = await adminApi.reviewContent(uploadId, action, rejectionReason);
      if (response.success) {
