...
const ResourceCard: React.FC<ResourceCardProps> = ({ resource, onClick, className = '' }) => {
  ...
  return (
    <div
      onClick={handleClick}
      className={`bg-white/90 backdrop-blur-md rounded-xl border border-brand-soft/20 shadow-md overflow-hidden hover:shadow-lg transition-all duration-200 cursor-pointer hover:border-brand-primary/50 ${className}`}
    >
      <div className="p-6">
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-3 flex-1 min-w-0">
            ...
            <div className="flex-1 min-w-0">
              ...
              {resource.category && (
                <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-brand-soft/10 text-brand-primary border border-brand-soft/30">
                  {resource.category}
                </span>
              )}
              {resource.author && (
                <p className="text-sm text-gray-600 mt-1 truncate">{t('resources.card.by')} {resource.author}</p>
              )}
            </div>
          </div>
        </div>
          ...
          <div className="mt-3 flex flex-wrap gap-1">
            ...
            {resource.tags.length > 3 && (
              <span className="inline-flex items-center px-2 py-1 bg-gray-100 text-gray-700 rounded-md text-xs">
                +{resource.tags.length - 3} {t('resources.card.more')}
              </span>
            )}
          </div>
        )}
