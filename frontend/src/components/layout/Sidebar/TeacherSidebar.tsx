...
const TeacherSidebar: React.FC<TeacherSidebarProps> = ({
  ...
  return (
    <div className={`flex flex-col h-full bg-gradient-to-br from-slate-50 via-purple-50/50 to-indigo-50/30 border-r border-gray-200/60 shadow-lg transition-all duration-300 ${isCollapsed ? 'w-16' : 'w-64'
      }`}>
      {/* Header - Compact */}
      <div className="flex items-center justify-between h-12 px-3 border-b border-slate-200 shadow-md" style={{ backgroundColor: brandColors.primaryHex }}>
        {!isCollapsed && (
          <div className="flex items-center space-x-2">
            ...
            <h1 className="text-sm font-bold text-white">{t('nav.teacher_portal', 'Teacher Portal')}</h1>
            ...
            <nav className={`space-y-1 ${isCollapsed ? 'px-1' : 'px-2'}`}>
              {adminItems.map((item) => {
                ...
                return (
                  ...
                  >
                    <div className={`flex-shrink-0 w-8 h-8 flex items-center justify-center rounded-lg ${active ? 'bg-gray-200' : 'bg-gray-100 group-hover:bg-gray-200'
        ...
        <div className="px-3 py-3 border-t border-gray-200/60 bg-gradient-to-r from-white/60 to-purple-50/60 backdrop-blur-sm">
          <div className="space-y-2">
            <div className="flex items-center justify-between text-xs">
              <span className="text-gray-500">{t('nav.stats.active_students', 'Active Students')}</span>
              <span className="font-semibold text-gray-900">247</span>
            </div>
            <div className="flex items-center justify-between text-xs">
              <span className="text-gray-500">{t('nav.stats.courses', 'Courses')}</span>
              <span className="font-semibold text-gray-900">8</span>
            </div>
            <div className="flex items-center justify-between text-xs">
              <span className="text-gray-500">{t('nav.stats.rating', 'Rating')}</span>
            ...
            <div className="flex items-center justify-between text-xs">
              <span className="text-gray-500">{t('nav.stats.revenue', 'Revenue')}</span>
              <div className="flex items-center space-x-1">
